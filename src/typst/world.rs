// This file is a very minor modification of typst-as-library
// (https://github.com/tfachmann/typst-as-library), licensed under Apache-2.0.
...
impl MinimalWorld {
    ...
    fn download_package(&self, package: &PackageSpec) -> PackageResult<PathBuf> {
        ...
        let raw_archive = DeflateDecoder::new(&compressed_archive)
            .decode_gzip()
            .map_err(|error| PackageError::MalformedArchive(Some(eco_format!("{error}"))))?;
        let mut archive = Archive::new(raw_archive.as_slice());
